...
feature_params = init_feature_params(features, feature_info);%CN特征（10->3）；hog特征(31->10)
...
% construct the Gaussian label function using Poisson formula(泊松分布)的极限分布是正态分布
...
if nScales > 0
    %force reasonable scale changes防止尺度过大或过小的变化
...
% show_precision(positions,seq.ground_truth,[])
