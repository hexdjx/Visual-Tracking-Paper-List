...
feature_params = init_feature_params(features);%CN特征（10->3）；hog特征(31->10)
...
% 二项分布与泊松分布都是离散分布，正态分布是连续分布。
% 二项分布的极限分布是泊松分布、泊松分布的极限分布是正态分布
...
%第一帧使用高斯-牛顿方法优化滤波器和投影矩阵，并初始化训练样本集
while true
    % Read image
    if seq.frame > 0
    ...
    if train_tracker     
        ...
        if seq.frame == 1 && params.update_projection_matrix
            ...
            if strcmp(params.distance_matrix_update_type, 'exact')
                ...
                for k = 1:num_feature_blocks %对样本内积求和
    ...
    positions(seq.frame,:) = [pos(2)-target_sz(2)/2,pos(1)-target_sz(1)/2,target_sz(2),target_sz(1)];
...
% precisions = show_precision(positions,seq.ground_truth,seq.video);

% fprintf('%12s - Precision (20px):% 1.3f, FPS:% 4.2f\n', seq.video, precisions(20), results.fps)
